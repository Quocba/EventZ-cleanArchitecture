...
using Base.Common;
using EventProduct.Application.Exceptions;
using Identity.API.Middlewares;
namespace EventProduct.API.Middlewares
{
    public class ApiResponseMiddleware(RequestDelegate next)
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            {
                await HandleExceptionAsync(context, originalBodyStream, HttpStatusCode.Conflict, ex.Message);
            }
            catch (Exception)
            {
